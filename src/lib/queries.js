...
const queries = {
    ...
    GET_ACTIVE_USER_BY_EMAIL_OR_ID: 'SELECT u.id, u.email, u.password, u.name, u.seed FROM scrypto.sc_user AS u WHERE (u.email = ${email} OR u.id = ${userId}) AND u.active = true;',
    ...
    DEACTIVATE_USER: 'UPDATE scrypto.sc_user SET active = false WHERE id = ${userId};'
};
